...
jQuery.fn.prettyGallery = function(settings) {
	...
	return this.each(function(){
		...
		var _formatGallery = function() {
			...
			galleryWidth = (itemWidth + itemMargin + 2) * settings.itemsPerPage - marginRight; // We don't want the margin of the last item, that's why we remove it.
		...
		var _applyNav = function() {
			var templateLeft = '';
				templateLeft +='<ul class="pg_paging">';
				templateLeft += '<li class="pg_previous"><a href="#" title="'+settings.previous_title_label+'">&nbsp;</a></li>'; /* '+settings.previous_label+' */
				templateLeft += '</ul>';
			
			var templateRight = '';
				templateRight +='<ul class="pg_paging">';
				templateRight += '<li class="pg_next"><a href="#" title="'+settings.next_title_label+'">&nbsp;</a></li>'; /* '+settings.next_label+' */
				templateRight += '</ul>';

			switch(settings.navigation){
				case 'top':
					$gallery.before(templateLeft);
					break;
				case 'bottom':
					$gallery.after(templateRight);
					break;
				case 'both':
					$gallery.before(templateLeft);
					$gallery.after(templateRight);
